...
const rawPos = params.get("pos");
const posFilter = rawPos
    ? rawPos.split(",").filter(Boolean)
    : [];
const fromParam = params.get("from");
const toParam   = params.get("to");

const from = fromParam ? Number(fromParam) : 1;
...
// 単語データ取得
// ===============================
let words = classicalWordData[bookId] ?? [];
...
let filteredWords = words
    ...
    .filter(word => {
        ...
        // meanset がない単語は除外（安全）
        if (!Array.isArray(word.meanset)) return false;

        // meanset 内に1つでも一致する品詞があればOK
        return word.meanset.some(item =>
            posFilter.includes(item.pos)
        );
    });
...
document.getElementById("result-close").onclick = () => {
    location.href =`study-classicalJapanese.html?book=${bookId}`
}
